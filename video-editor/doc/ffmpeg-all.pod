...
B<native experimental FFmpeg AAC encoder>, according to
multiple sources.
...
C<-2.0> to C<2.0>. The default value is "0".
...
directories specified by the colon-separated list in B<FREI0R_PATH>.
...
compose with B<overlay>. In this example the video will gradually
...
Set step for C<waveform> mode. Smaller values are useful to find out how
...
=item B<waveform_mirror>

Set mirroring mode for C<waveform>. C<0> means unmirrored, C<1>
...
C<row> waveform mode or one below the other in C<column> waveform mode
for C<waveform> histogram mode. For C<levels> histogram mode,
per color component graphs are placed below each other.

Using this display mode in C<waveform> histogram mode makes it easy to
...
This display mode in C<waveform> histogram mode makes it easier to spot
...
Set mode for C<levels>. Can be either C<linear>, or C<logarithmic>.
Default is C<linear>.


=item B<components>

Set what color components to display for mode C<levels>.
Default is C<7>.
...
Default value is C<384/f*tc/(384/f+tc)>.
...
(such as bass guitar). Acceptable value is [0.1, 1.0]. Default value is C<0.17>.
...
Specify video fps. Default value is C<25>.
